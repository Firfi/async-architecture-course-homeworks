...
import { assertExists, assertNonEmptyAndAssigned, castSome } from '@monorepo/utils';
...
const unauthorizedResponse = async (req: Request, res: Response) => {
  const authURL = await fiefClient.getAuthURL({ redirectURI: REDIRECT_URI, scope: ['openid'] });
...
app.get('/protected', fiefAuthMiddleware(), (req, res) => {
  res.send(`<h1>You are authenticated. Your user email is ${assertExists(req.user).email}</h1>`)
});
...
app.get('/authenticated-scope', fiefAuthMiddleware({ scope: ['openid', 'required_scope'] }), (req, res) => {
  res.json(req.accessTokenInfo);
});

app.get('/authenticated-permissions', fiefAuthMiddleware({ permissions: ['castles:read'] }), (req, res) => {
  res.json(req.accessTokenInfo);
});
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      if (topic === USER_TOPIC_NAME) {
        const user = pipe(message.value, assertExists, v => v.toString(), JSON.parse, S.parseSync(User));
