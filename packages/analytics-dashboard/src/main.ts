...
app.get(
  '/winnerPrice/:dayFrom/:dayTo',
  fiefAuthMiddleware(),
  useCanRole([ROLE_ADMIN]),
  (req, res) => {
    const dayFrom = new Date(req.params.dayFrom);
    const dayTo = new Date(req.params.dayTo);
    res.json({
      maxPrice: Number(
        getMaxPriceForInterval(dayFrom, dayTo)
      ) /*TODO assert number cast but generally we don't care*/,
    });
  }
);
